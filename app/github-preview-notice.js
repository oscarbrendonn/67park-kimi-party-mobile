...
const poolDeadline=performance.now()+30000;
const installPool=()=>{const world=window.__islandWorld;if(world?.ready&&world.terrain){const result=applySmallParkPool(world.terrain);world.smallParkPool=result;world.renderer?.domElement&&(world.renderer.domElement.dataset.smallParkPool=JSON.stringify(result));return}if(performance.now()<poolDeadline)setTimeout(installPool,60)};
installPool();

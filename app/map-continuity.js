...
export function applyMapContinuity(root,patch){
 if(root.userData.mapContinuity)return root.userData.mapContinuity;
 if(patch?.version!==59||patch.metrics.grassWaterOverlap>1e-6||patch.metrics.pathWaterOverlap>1e-6)throw Error('Invalid map continuitys');
 root.updateMatrixWorld(true);
 const prepared=[];
 for(const row of patch.meshes){
  const mesh=root.getObjectByName(row.name),old=mesh?.geometry;
  if(!old||old.attributes.position.count!==row.expected.vertices||old.index?.count!==row.expected.indices)throw Error('Map continuity source changed: '+row.name);
  const remove=new Set(row.remove);
  if(remove.size!==row.remove.length||row.remove.some(i=>i%3||i<0||i>=old.index.count)||row.p.length!==row.n.length||row.p.length%3||!row.p.every(Number.isFinite)||!row.n.every(Number.isFinite)||row.ix.some(i=>!Number.isInteger(i)||i<0||i>=row.p.length/3))throw Error('Invalid map continuity geometry');
